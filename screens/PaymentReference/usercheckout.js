...
function getSignature(amount, id, email, transactionType) {
    ...
    if (transactionType) {
        ...
        $('input[name="id"]').val(id + "adv");
    } else {
        $('input[name="order_description"]').val(id + "Item");
        $('input[name="id"]').val(id);
        $('input[name="merchant_reference"]').val(id);
    }
    ...
    console.log("signature is : ",sha256sig);
...
function getToken() {
    ...
    callApi.getUrl("http://api.homegenie.me:8007/api/payment/userCheckout?" + url, "", function(err, response){
            if (!err && response) {
                    response = response.data;
                    console.log("response is : ", response);
                    // $("#paymentpage").html(response);
                    getSignature(response.amount, response.id, response.email, response.transactionType);
                }
            });
